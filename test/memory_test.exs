defmodule MemoryTest do
  ...
  test "parse_line/1" do
    ...
    l4 = ~r(336 <45>1 2015-05-12T01:45:26.295061+00:00 heroku web.1 - - source=web.1 dyno=heroku.34921046.7aef9231-b8f6-428b-8bc0-60689bb2b584 sample#memory_total=33.34MB sample#memory_rss=32.88MB sample#memory_cache=0.46MB sample#memory_swap=0.00MB sample#memory_pgpgin=9971pages sample#memory_pgpgout=1435pages)

    assert ParseLogs.Base64Video.parse_line(l1) == "1431452801,381"
    assert ParseLogs.Base64Video.parse_line(l2) == "1431452801,425"
    assert ParseLogs.Base64Video.parse_line(l3) == "1431452801,115"
    assert ParseLogs.Base64Video.parse_line(l4) == "1431452801,33"
  end
end
